...
//.setTimestamp()
//.setFooter("© 2020 Developed by EmreeK-Odinex🔸#5614", client.user.avatarURL())
//guild.send(embed)
//})
...
// ------------------------------yeni sunucuya eklenince rastgele mesaj ------------------------------
client.on('guildCreate', guild => {
...
// ------------------------------yeni sunucuya eklenince rastgele mesaj bitiş------------------------------
...
//.addField(`Discorda katılma tarihi`, ` \`${member.user.createdAt.toLocaleString()}\` `)
//.setTimestamp()
...
client.on("guildMemberAdd", member => {
    ...
    var kanal = db.fetch(`otorolkanali_${member.guild.id}`)
    var kanalcık = member.guild.channels.cache.get(kanal)
    var yazı = db.fetch(`otorolyazi_${member.guild.id}`)
    if(!yazı){
      var yazı = "" 
    }
